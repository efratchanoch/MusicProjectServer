...
public class SheetAnalysisAgentService {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            "instruments":
                ...
                Use ONLY general instrument names (e.g., Piano, Guitar, Violin, Clarinet, Flute, Trumpet).
        ...
        public SheetAnalysisAgentService(ChatClient.Builder builder,
                                         ...
                                         PdfTextExtractorService pdfTextExtractorService // ⬅️ נוסף!
        ) {
            this.musicMetadataSearchTool = musicMetadataSearchTool;
            this.pdfTextExtractorService = pdfTextExtractorService; // ⬅️ שמירה בשדה

            this.chatClient = builder
                    .defaultSystem(SYSTEM_PROMPT)
                    .defaultTools(musicMetadataSearchTool) // ⬅️ הכלי כבר מופעל
                    .build();
        }
        ...
        public SheetMusicResponseAI analyzePdfBytes(byte[] pdfBytes) throws Exception {
            ...
            // 1️⃣ חילוץ הטקסט והכותרת הפנימית באמצעות השירות שלך (OCR/PDF Stripper)
            ...
            if (extractedText.length() < 50) {
                // אם החילוץ נכשל, נסה Base64 קטן אם זה הכרחי, או זרוק שגיאה
                throw new RuntimeException("Failed to extract sufficient text from PDF/OCR.");
            }

            // 2️⃣ בניית ה-Prompt (שימוש בטקסט שחולץ במקום ב-Base64 המלא)
            ...
            // ... המשך הקוד כרגיל (קריאה ל-chatClient וניתוח ה-JSON) ...
...
//
//        String prompt = "Analyze the following PDF (base64). " +
//                "**Crucial Instruction: If 'composer' or 'lyricist' are missing from the PDF text, you must first call the 'searchForMetadata' tool using the extracted 'title' before finalizing the JSON response.** " +
//                "Return JSON ONLY with all required fields. PDF follows:\\nPDF_BASE64_START\\n"
//                + base64 + "\\nPDF_BASE64_END";
//
//        UserMessage userMessage = new UserMessage(prompt);
//
